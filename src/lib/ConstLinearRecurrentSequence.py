...
class ConstLinearRecurrentSequence(BitSet.BitSet):
    """docstring for ConstLinearRecurrentSequence"""

    def __init__(self, Length, Initial, Polynomial):
        # print("init ConstLinearRecurrentSequence")
        super(ConstLinearRecurrentSequence, self).__init__(Length)
        ...
        for i in range(self.__Length__):
            if (reg & 1):
                super(ConstLinearRecurrentSequence, self).set(i+self.__start_index__)
            else:
                super(ConstLinearRecurrentSequence, self).reset(i+self.__start_index__)
            accum = 0
            for j in range(32):
    ...
    def getValue(self):
        return super(ConstLinearRecurrentSequence, self).test(self.__index__)
        
def main():
    # CA code generator example
    G =  ConstLinearRecurrentSequence(1023,0x3F,(1<<5)|(1<<3))
    G1 = ConstLinearRecurrentSequence(1023,0x3FF,(1<<10)|(1<<3))
    G2 = ConstLinearRecurrentSequence(1023,0x3FF,(1<<10)|(1<<9)|(1<<8)|(1<<6)|(1<<3)|(1<<2))
