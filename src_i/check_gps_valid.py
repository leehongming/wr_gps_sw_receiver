...
class Search(object):
    ...
    def process(self,show_input_freq_hist=False):
        ...
        if show_input_freq_hist:
            input_fd = pyfftw.empty_aligned(self.num_samples//2+1, dtype='complex128')
            fft_input = pyfftw.FFTW(numpy.float64(input_td), input_fd,direction="FFTW_FORWARD",flags=('FFTW_ESTIMATE',),threads=2)
            fft_input_result = fft_input()
            plt.plot(abs(fft_input_result))
            plt.show()
        ...
        fft_code = pyfftw.FFTW(code_td, code_fd,direction="FFTW_FORWARD",flags=('FFTW_ESTIMATE',),threads=1)
        fft_code_result = fft_code()
        code_fd_conj = numpy.conj(code_fd)

        mix_td = pyfftw.empty_aligned(self.num_samples, dtype='complex128')
        mix_fd = pyfftw.empty_aligned(self.num_samples, dtype='complex128')
        ...
        for i in range(self.bins):
            ...
            for i in range(self.num_samples):
                ...
                carrier = 7*complex(carrier_cos,carrier_sin)
                # mix the input signal with local carrier
                mix_td[i] = carrier * input_td[i]
                carrier_nco.tick()
        
            # Convert to frequency domain.
            fft_mix = pyfftw.FFTW(mix_td, mix_fd,direction="FFTW_FORWARD",flags=('FFTW_ESTIMATE',),threads=1)
            fft_mix_result = fft_mix()
            # Multiply conjugate of input frequency samples by local code replicas samples (point by point).
            multi_fd=(mix_fd*code_fd_conj)/self.num_samples
